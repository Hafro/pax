...
if (!exists("mar")) mar <- mar::connect_mar()

species_code <- 2
import_defs <- list(mar, species = species_code, year_start = 1990, year_end = 1994)
...
ok_group("R/01-plots_and_tables.R:sampling_position", {
  df_tidypax <- tidypax:::sampling_position(mar,species_nr = species_code, year_range = import_defs$year_start:import_defs$year_end) |>
...
ok_group("R/01-plots_and_tables.R:sampling_tables", {
  ...
  df_tidypax <- tidypax:::sampling_tables(mar,species_nr = species_code) |>
    ...
    dplyr::collect() |> dplyr::rename(year = Year)
  df_newpax <- dplyr::tbl(pcon, "sampling") |>
    dplyr::left_join(dplyr::tbl(pcon, "measurement"), by = "sample_id") |>
    pax_sampling_detail() |>
    dplyr::relocate("year") |> 
...
ok_group("R/01-plots_and_tables.R:catch_agg", {
  df_tidypax <- suppressWarnings(tidypax::catch_data(mar,species_code) |>
    dplyr::filter(year >= local(import_defs$year_start), year <= local(import_defs$year_end)) |>
  ...
  df_newpax <- dplyr::tbl(pcon, "si") |>
    ...
    dplyr::inner_join(
      dplyr::tbl(pcon, "measurement") |>
        dplyr::filter(measurement_type == "OTOL", !is.na(age), !is.na(maturity_stage)) |>
    ...
    pax_add_regions(regions = list(S=c(101,107,106,108,109,114),
                                         N = c(102,103,104,105,111,113)), default="S") |>
    dplyr::group_by(year,lgroup,age,region) |>
    dplyr::summarise(p = mean(mat)) |>
    dplyr::arrange(year, region, age, lgroup) |>
    dplyr::collect()
