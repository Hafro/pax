...
pax_dat_gridcell <- function (pcon, gridcells) {
  ...
  if (tbl_name %in% DBI::dbListTables(pcon)) {
    ...
    if (length(gridcells) > 0) DBI::dbWriteTable(
      pcon,
      tbl_name,
      get_rows(gridcells),
      append = TRUE
    ...
    pax_import(pcon, get_rows(gridcells), name = tbl_name, cite = list(
      citation("marmap"),
      citation("geo")
    ))
  }
...
pax_dat_sampling_type_desc <- function(pcon) dat_from_pkg(pcon, "sampling_type_desc")
...
pax_dat_mfdb_gear_code_desc <- function(pcon) dat_from_pkg(pcon, "mfdb_gear_code_desc")
